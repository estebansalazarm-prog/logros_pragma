...
def get_table_transformations(
    ...
    # Agregar columna de id que sera la llave primaria de la tabla
    if len(key_columns) >= 2:
        ...
        df_latest_status = df_latest_status.withColumn(concat_col_name, F.concat_ws("-", *cols_to_concat))

    # Usar schema_name del YAML
    ...
    if job_config.elt_config_table.processing_config.post_read_filters:
        for filter_str in job_config.elt_config_table.processing_config.post_read_filters:
            df_aligned_schema = df_aligned_schema.filter(filter_str)

    if job_config.elt_config_table.processing_config.rename_columns:
        for rename in job_config.elt_config_table.processing_config.rename_columns:
            df_aligned_schema = df_aligned_schema.withColumnRenamed(rename.old_name, rename.new_name)

    if job_config.elt_config_table.processing_config.sql_transformations:
        for sql_transformation in job_config.elt_config_table.processing_config.sql_transformations:
